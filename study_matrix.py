...
def generate_evaluations(rows, columns, number_of_evaluations, eval_func, **kwargs):
    evaluations = []
    for _ in range(number_of_evaluations):
        evaluations.append(eval_func(rows, columns, **kwargs))
    return evaluations
...
def main():
    ...
    to_eval.append(("device_evaluation", generate_evaluations(models1, devices, 5, generate_study_matrix_models_latin_rectangle)))



    guidance_scales = [1, 2, 3, 4, 5]
    to_eval.append(("guidance_scale_evaluation", generate_evaluations(models1, guidance_scales, 3, generate_study_matrix_models_latin_rectangle)))


    inference_steps = [5, 10, 20, 50, 100, 200, 400]
    to_eval.append(("inference_steps_evaluation", generate_evaluations(models1, inference_steps, 3, generate_study_matrix_models_latin_rectangle)))


    length = [5, 10, 20, 30]
    models2 = ['audioldm-m-full', 'audioldm2']
    to_eval.append(("length_evaluation", generate_evaluations(models2, length, 3, generate_study_matrix_models_latin_rectangle)))
    ...
    to_eval.append(("negative_prompts_evaluation", generate_evaluations(models3, negative_prompts, 3, generate_study_matrix_models_random)))
    ...
    to_eval.append(("single_events_evaluation", generate_evaluations(prompts_single_events, models4, 1, generate_study_matrix_models_latin_rectangle, shuffle_rows=False)))

    prompts_instruments = ['FM synthesis bells', 'mellotron chords', 'A bagpipe melody', 'A guitar string', 'A piano chord']
    to_eval.append(("instruments_evaluation", generate_evaluations(prompts_instruments, models4, 1, generate_study_matrix_models_latin_rectangle, shuffle_rows=False)))
    ...
    to_eval.append(("adjectives_evaluation", generate_evaluations(prompts_adjectives, models4,  1, generate_study_matrix_models_latin_rectangle, shuffle_rows=False)))
    ...
    to_eval.append(("effect_evaluation", generate_evaluations(prompts_effect, models4, 1, generate_study_matrix_models_latin_rectangle, shuffle_rows=False)))
    ...
    to_eval.append(("music_production_evaluation", generate_evaluations(prompts_music_production, models4, 1, generate_study_matrix_models_latin_rectangle, shuffle_rows=False)))


    # Save the evaluation matrix to a PDF
    save_evalauation_to_pdf(to_eval, "evaluation.pdf")
